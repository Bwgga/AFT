...
const int buttonPin = 2; //S on the button
const int ledRed = 6;
...
void loop() {
  ...
  if (digitalRead(buttonPin) == HIGH) { //if button high it isn't pressed
    currState = false;
  }
  else {
    currState = true;
    //check if the state of the button changed
...
*/
  ...
  prevState = currState; //set the current state as previous state for the next loop
  delay(100);
}